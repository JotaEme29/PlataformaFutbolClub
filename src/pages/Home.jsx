...
const RegistroForm = () => {
  ...
  return (
    <div className="w-full text-gray-900 dark:text-gray-100">
      <h2 className="text-center text-2xl font-bold mb-1">Registrar mi Club</h2>
      <p className="text-center text-gray-500 dark:text-gray-400 mb-6">Únete a la plataforma de gestión definitiva</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block mb-1 font-semibold text-gray-700 dark:text-gray-300" htmlFor="nombreClub">Nombre del Club</label>
          <input className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="text" id="nombreClub" name="nombreClub" value={formData.nombreClub} onChange={handleChange} required />
        </div>
        <div>
          <label className="block mb-1 font-semibold text-gray-700 dark:text-gray-300" htmlFor="email">Correo electrónico</label>
          <input className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="email" id="email" name="email" value={formData.email} onChange={handleChange} required />
        </div>
        <div>
          <label className="block mb-1 font-semibold text-gray-700 dark:text-gray-300" htmlFor="password">Contraseña (mín. 6 caracteres)</label>
          <input className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="password" id="password" name="password" value={formData.password} onChange={handleChange} required />
        </div>
        <div>
          <label className="block mb-1 font-semibold text-gray-700 dark:text-gray-300" htmlFor="confirmPassword">Confirmar Contraseña</label>
          <input className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" type="password" id="confirmPassword" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} required />
        </div>
        <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-md font-semibold hover:bg-blue-700 transition-colors duration-300 mt-4 disabled:bg-blue-400" disabled={loading}>
          {loading ? 'Registrando...' : 'Crear Club'}
        </button>
      </form>
    </div>
  );
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-white dark:bg-gray-800">
          ...
          <div className="flex mb-8 rounded-xl bg-gradient-to-r from-green-100 via-blue-100 to-purple-100 dark:from-gray-700 dark:via-gray-700 dark:to-gray-700 p-1.5 shadow-inner">
            <motion.button 
              ...
              className={`w-1/2 py-3 text-sm font-bold rounded-lg transition-all duration-300 ${
                authMode === 'login' 
                  ? 'bg-gradient-to-r from-green-600 via-blue-600 to-purple-600 text-white shadow-lg transform scale-105' 
          ...
          {authMode === 'login' ? <LoginForm /> : <RegistroForm />}
        </div>
      </motion.div>
